...
tab_if, tab_mistral = st.tabs(["🔍 Détection d'anomalies", "💬 Mistral AI"])
...
with tab_if:
    st.markdown(
        ...
        "(scans de ports, tentatives de brute-force, trafic inhabituel)."
    )
    ...
    with col_p:
        contamination = st.slider(
            ...
            help="Proportion attendue de flux anormaux dans les données (5% par défaut)"
        )
    ...
    with col_r:
        ...
        if ratio > 20:
            st.warning(
                ...
                "Vérifier si les données contiennent un événement de sécurité réel "
                "ou réduire le paramètre 'Taux d'anomalie estimé'."
            )
        else:
            st.info(
                f"ℹ️ **{n_anom:,} flux atypiques ({ratio:.1f}%)** identifiés. "
                "Consulter le tableau ci-dessous pour investigation."
            )

        # Scatter scores
        sample = df_res.sample(min(2000, len(df_res)), random_state=42)
        fig = px.scatter(
            sample, x="dst_port", y="anomaly_score",
            color="anomaly",
            color_discrete_map={True: "#EF4444", False: "#3B82F6"},
            hover_data=["src_ip", "dst_ip", "proto", "action"],
            labels={"dst_port": "Port destination",
...
with tab_mistral:
    st.markdown(
        "**Mistral AI** analyse les statistiques du dataset et génère un rapport "
        "de sécurité en langage naturel."
    )

    # Clé API — .env ou saisie manuelle
    api_key = os.getenv("MISTRAL_API_KEY", "")
    if not api_key:
        st.info(
            "💡 Ajouter `MISTRAL_API_KEY=votre_clé` dans le fichier `.env` "
            "pour éviter de la saisir à chaque session.",
            icon="🔑",
        )
        api_key = st.text_input(
            "Clé API Mistral", type="password",
    ...
    if st.button("🧠 Générer le rapport", type="primary",
        ...
        prompt = f"""Tu es un expert en cybersécurité réseau. Analyse ces statistiques iptables et rédige un rapport concis en français.

DONNÉES ({len(df):,} flux) :
- Taux de Deny : {deny_p}%
- Protocoles : {proto_cnt}
- TOP 5 IPs sources : {top5_ips}
- TOP 5 ports ciblés : {top5_ports}
- Flux depuis IPs hors plan interne : {n_ext}
